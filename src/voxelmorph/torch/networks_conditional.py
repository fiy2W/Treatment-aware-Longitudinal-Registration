...
class VxmDense(LoadableModel):
    ...
    def net(self, reg_code, source_img, target_img):
        ...
        # low resolution
        e_lr1 = self.lr_c1(lr)
        x_lr = self.lr_c2(e_lr1)
        for i in range(len(self.resblock_group_lr)):
            if i % 2 == 0:
                x_lr = self.resblock_group_lr[i](x_lr, reg_code)
            else:
                x_lr = self.resblock_group_lr[i](x_lr)
        x_lr = self.upsample(x_lr)
        x_lr = self.lr_uc1(torch.cat([x_lr, e_lr1], dim=1))
        x_lr = self.upsample(x_lr)
        x_lr = self.lr_uc2(torch.cat([x_lr, lr], dim=1))
        x_lr = self.upsample(x_lr)

        flow_lr = self.flow_lr(x_lr)

        # mid resolution
        e_mr1 = self.mr_c1(mr)
        x_mr = self.mr_c2(e_mr1)
        for i in range(len(self.resblock_group_mr)):
            if i % 2 == 0:
                x_mr = self.resblock_group_mr[i](x_mr, reg_code)
            else:
                x_mr = self.resblock_group_mr[i](x_mr)
        x_mr = self.upsample(x_mr)
        x_mr = self.mr_uc1(torch.cat([x_mr, e_mr1], dim=1))
        x_mr = self.upsample(x_mr)
        x_mr = self.mr_uc2(torch.cat([x_mr, mr], dim=1))
        #x_mr = self.upsample(x_mr)

        x_mr = self.lr_merge(torch.cat([x_mr, x_lr], dim=1))
        flow_mr = self.flow_mr(x_mr)
        ...
        x_hr = self.mr_merge(torch.cat([x_hr, x_mr], dim=1))
        flow_hr = self.flow_hr(x_hr)
        return flow_lr, flow_mr, flow_hr
