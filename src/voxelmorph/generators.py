...
def scan_to_scan(vol_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    ...
    while True:
        ...
        img_X = scan1[0,:,:,:,0]
        img_Y = scan2[0,:,:,:,0]
        rt = random.randint(0, 6)
        if rt == 0:
            pass
        elif rt == 1:
            img_X = np.transpose(img_X, axes=(0,2,1))
            img_Y = np.transpose(img_Y, axes=(0,2,1))
        elif rt == 1:
            img_X = np.transpose(img_X, axes=(1,0,2))
            img_Y = np.transpose(img_Y, axes=(1,0,2))
        elif rt == 1:
            ...
            img_X = np.transpose(img_X, axes=(2,1,0))
            img_Y = np.transpose(img_Y, axes=(2,1,0))
        rf = random.randint(0,7)
        if rf//4 == 1:
            img_X = img_X[::-1,:,:]
            img_Y = img_Y[::-1,:,:]
        if (rf%4)//2 == 1:
