        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                loss_similar = mi.loss(out['lr_img'][:,0:1,:,:,:], fix_down[:,0:1,:,:,:]) + \
                    ...
                    nn.MSELoss()(out['lr_img'][:,0:1,:,:,:], fix_down[:,0:1,:,:,:]) + \
                    nn.MSELoss()(out['mr_img'][:,0:1,:,:,:], fix_down[:,0:1,:,:,:]) + \
                    nn.MSELoss()(out['hr_img'][:,0:1,:,:,:], fix_down[:,0:1,:,:,:]) + \
                    nn.MSELoss()(out['lr_img_f'][:,0:1,:,:,:], fix[:,0:1,:,:,:]) + \
                    nn.MSELoss()(out['mr_img_f'][:,0:1,:,:,:], fix[:,0:1,:,:,:]) + \
                    nn.MSELoss()(out['hr_img_f'][:,0:1,:,:,:], fix[:,0:1,:,:,:])
                
                loss_seg = nn.L1Loss()(out['lr_seg'], fix_seg_down) + vxm.torch.losses.DiceLoss()(out['lr_seg'], fix_seg_down) + \
                    nn.L1Loss()(out['mr_seg'], fix_seg_down) + vxm.torch.losses.DiceLoss()(out['mr_seg'], fix_seg_down) + \
                    nn.L1Loss()(out['hr_seg'], fix_seg_down) + vxm.torch.losses.DiceLoss()(out['hr_seg'], fix_seg_down)
                ...
                loss = 1 * loss_similar + 1 * loss_seg + 10 * loss_grad
...
def get_args():
    parser = argparse.ArgumentParser(description='Train CPRN model',
    ...
    parser.add_argument('-d', '--device', dest='device', type=str, default='cpu',
                        help='cuda or cpu')
    parser.add_argument('-s', '--save', dest='save', type=str, default='ckpt/cprn',
                        help='save ckpt')
